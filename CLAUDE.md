...
## What is this?
C++17 implementation of the Antheos Protocol v1.0 Level 1 specification.
...
make test         # Build and run all test suites (201 tests)
...
- `tests/` — Test suites (9 files + test_common.hpp, 201 tests)
...
Namespaces:
  ...
  id::        Base-32 encoding, BID/SID generation
...
Classes:
  ...
  Context     Stateful codec — owns identity, SidPool, Parser, 32 sessions (pimpl)
```
...
- Context throws `std::invalid_argument` on construction with empty identity
...
| test_identity.cpp | 12 | Base-32 encoding, BID/SID generation, SID pool lifecycle |
...
| test_context.cpp | 43 | High-level API: lifecycle, sessions, feed+callbacks |

## Rules for modifications
- Run `make clean && make test` before every commit — all 201 tests must pass
