...
{
    Context ctx("ACME", "Echo", "SN001", "ABCDEFGH");
...
{
    bool threw = false;
    try { Context ctx("", "Echo", "SN001", "ABCDEFGH"); }
...
{
    bool threw = false;
    try { Context ctx("ACME", "", "SN001", "ABCDEFGH"); }
    catch (const std::invalid_argument&) { threw = true; }
    ASSERT_TRUE(threw);
}
...
{
    bool threw = false;
    try { Context ctx("ACME", "Echo", "", "ABCDEFGH"); }
    catch (const std::invalid_argument&) { threw = true; }
    ASSERT_TRUE(threw);
}
...
{
    bool threw = false;
    try { Context ctx("ACME", "Echo", "SN001", ""); }
    catch (const std::invalid_argument&) { threw = true; }
    ASSERT_TRUE(threw);
}
...
static void test_verify_response(void)
{
    ...
    ASSERT_TRUE(frame_valid(*f));
    ASSERT_TRUE(frame_has_verb(*f, 'V'));
    ASSERT_TRUE(frame_has_id_plain(*f, "ACME"));
    ASSERT_TRUE(frame_has_id_plain(*f, "Echo"));
...
static void test_feed_verify_response(void)
{
    ...
    Context remote("REMOTE", "Agent", "SN042", "REMOTE01");
    ...
    ASSERT_TRUE(state.last_id == "REMOTE");        /* OID */
    ASSERT_TRUE(state.last_id2 == "Agent");        /* DID */
